...
- [ ] **Wk 1: Analytics.** There is currently NO analytics in `src/`. Install GA4 (free) via a small script component, with events for: quote-modal open, quote-modal submit, `/free-ai-audit` form submit, (later) calculator completion. Without this, "leads by source" in `11_METRICS_AND_TRACKING.md` is fiction.
